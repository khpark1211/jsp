	...
	<table>
			...
			<c:choose>
				...
				<c:forEach var="k" items="${list}" varStatus="vs">
					<tr>
						...
						<a href="/MyController?cmd=view&idx=${k.idx}&cPage=${paging.nowPage}">${k.title}</a>
						</td>
						<td>${k.writer }</td>
		...
		<tfoot>
			<tr>
				<td colspan="4">
					...
					<c:choose>
						<c:when test="${paging.beginBlock > paging.pagePerBlock }">
							<a href="/MyController?cmd=list&cPage=${paging.beginBlock-paging.pagePerBlock}"> 
					...
					<c:forEach begin="${paging.beginBlock}" end="${paging.endBlock}" step="1" var="k">
						<c:choose>
							...
							<c:otherwise>
								<span style="background:#B2CCFF "><a href="/MyController?cmd=list&cPage=${k}"><font size="4">${k}</a></font></span>&nbsp;&nbsp;
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<!-- 다음 -->
				...
				<td>
					<img  src="../img/but_write.gif" onclick="write_go()">
				</td>
			</tr>
		</tfoot>
	</table>
